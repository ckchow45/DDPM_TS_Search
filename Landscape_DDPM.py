...
class GaussianDiffusion(nn.Module): # defining a GaussianDiffusion class from the pytorch module, contains the forward diffusion process, noise sampling and loss prediction
    ...
    def q_sample(self, x_start, t, noise=None): # forward diffusion process = adding noise to the original data at timestep (t)
        ...
        sqrt_alphas_cumprod_t = self._extract(self.sqrt_alphas_cumprod, t, x_start.shape) # Fetch ᾱ and 1-ᾱ terms for each sample in the batch based on t. Shapes are broadcast to match x_start.
    ...
    def _extract(self, a, t, x_shape): # Helper function to index into a 1D tensor (a) using the batch of timestep indices t, then reshape for broadcasting.
        """
        Extract values from a for batch of indices t and reshape to x_shape.
        """
        b = t.shape[0]
        out = a.gather(-1, t).float() # gather() extracts a[t[i]] for each i, giving shape (B,).
        return out.view(b, *((1,) * (len(x_shape) - 1))) # view() reshapes it to (B, 1, etc.) to match x_shape and allow element-wise multiplication.
...
class UNet(nn.Module):
    def __init__(self, input_dim, base_dim=64, dim_mults=(1, 2, 4), time_emb_dim=128, out_dim=None):
        ...
        nn.Linear(input_dim * 2, time_emb_dim), # takes a concatenated input of start, end and path and compresses the data
        Mish(),
        ...
        for i in reversed(range(len(dims) - 1)): # iterate for every dim, allowing modular number of layers in the Unet
            self.ups.append(nn.ModuleList([ # add to list of layers
                ResnetBlock(dims[i + 1] + dims[i + 1], dims[i], time_emb_dim), # Apply ResNet block
    ...
    def forward(self, x, t, start, end):
        ...
        # concat along feature dimension (F)
        ...
        t_emb = t_emb + se_emb                   # put start/end into timestep embedding so model understands what the start and end point in time looks like
